...
import net.gfxmonk.capsul.StagedFuture.Implicits._
...
object StateBased {
	...
	class Prefixer(next: Option[Prefixer]) {
		...
		def prefix(value: String): StagedFuture[String] = {
			...
			state.accessAsync[Future[String]](prefix => {
	...
	def applyMultiplePrefixesAsync(value: String): Future[String] = {
		...
		for {
			...
			() <- b.initPrefix("then b ")
			() <- c.initPrefix("then c:")
			result <- c.prefixAsync(value).flatten
		} yield result
	}
}
