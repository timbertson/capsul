...
import net.gfxmonk.capsul.UnitOfWork
...
object Capsul {
	...
	def apply[T](v: T)(implicit ec: ExecutionContext) =
		new Capsul[T](v, BackpressureExecutor())

	/** Create a Capsul with the provided bufLen */
	def apply[T](v: T, bufLen: Int)(implicit ec: ExecutionContext) =
		new Capsul[T](v, BackpressureExecutor(bufLen))

	def apply[T](v: T, thread: BackpressureExecutor)(implicit ec: ExecutionContext) =
...
completes)
 ...
 - '''staged''':  async ([[StagedFuture]]). The work returns a [[StagedFuture]][T], and
	...
	*/
class Capsul[T](init: T, thread: BackpressureExecutor) {
...
//		thread.enqueue(() => state.set(updated))
//
